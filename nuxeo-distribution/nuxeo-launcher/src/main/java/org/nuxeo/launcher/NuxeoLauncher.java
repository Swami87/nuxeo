...
public abstract class NuxeoLauncher {
    ...
    public String promptEmail() throws IOException, ConfigurationException {
        ...
        if (console != null) {
            email = console.readLine(message);
            while (email == null || !validator.isValid(email)) {
                console.printf(error + "\n", email);
                email = console.readLine(message);
            }
        } else { // try reading from stdin
            email = IOUtils.toString(System.in);
     ...
     * @param regex A regular expression the user input must match. Ignored if {@code null} or blank
     ...
     * @since 8.3
     */
    public String prompt(String message, String regex, String error) throws ConfigurationException, IOException {
        boolean doRegexMatch = StringUtils.isNotBlank(regex);
        String value = null;
        Console console = System.console();
        if (console != null) {
            value = console.readLine(message);
            while (value == null || doRegexMatch && !value.matches(regex)) {
                console.printf(error + "\n", value);
                value = console.readLine(message);
            }
        } else { // try reading from stdin
            value = IOUtils.toString(System.in);
            if (value == null || doRegexMatch && !value.matches(regex)) {
                throw new ConfigurationException(error);
            }
        }
        return value;
    ...
    public boolean registerRemoteInstance() throws IOException, ConfigurationException, PackageException {
        ...
        if (params.length > 0) {
            ...
            username = prompt("Username: ", "^\\w+", "Username cannot be empty.");
        }
        char[] password;
    ...
    public boolean registerTrial() throws IOException, ConfigurationException, PackageException {
        ...
        if (params.length > 3) {
            ...
            if (!Pattern.matches("^\\w+$", company)) {
                throw new ConfigurationException("Company field is mandatory.");
            }
            registration.put("company", company);

            String projectName = params[2];
            if (!Pattern.matches("^(?:[-\\w]+|)$", projectName)) {
            ...
            registration.put("description", params[3]);
        } else {
            registration.put("email", promptEmail());
            registration.put("company", prompt("Company: ", "^\\w+$", "Company field is mandatory."));
            registration.put(
                    "connectreg:projectName",
                    prompt("Project name: ", "^(?:[-\\w]+|)$",
                            "Project name can only contain alphanumeric characters and dashes."));
            registration.put("description", promptDescription());
        }

        if (params.length == 5) {
            registration.put("password", params[4]);
            registration.put("password_verif", params[4]);
        } else {
            // TODO NXP-19258 remove password_verif
            char[] pwd = promptPassword(true);
            registration.put("password", new String(pwd));
            registration.put("password_verif", new String(pwd));
        }
        ...
        log.info(String.format(
                ...
                registration.get("email"), registration.get("connectreg:projectName"), registration.get("description")));
        return true;
    }
